...
Brand colors are now CSS variable-driven and admin-editable. The two gradient stops (`--brand-primary`, `--brand-accent`) are defined in `client/src/index.css` for both `:root` and `.dark`, and overridden at runtime by a server-rendered stylesheet. The server exposes `GET /api/theme.css` (cached as `no-cache`), which reads `theme.brandPrimary`, `theme.brandAccent`, `theme.brandPrimaryDark`, `theme.brandAccentDark` from `site_settings` and emits a tiny CSS file. `client/index.html` references it via `<link rel="stylesheet" href="/api/theme.css">` so the browser blocks paint until the theme is applied (no FOUC). All previously hard-coded `hsl(190,85%,55%)` / `hsl(220,90%,60%)` references across home, admin, insights, ChatBot, SectionNavigation, SiteContentTabs and App were refactored to `hsl(var(--brand-primary|--brand-accent))`. A new admin "Appearance" tab (`AppearanceAdmin`) provides color pickers (with HSL text fallback), live gradient preview, Save (PUT `/api/site/settings`) and Restore Defaults; on save the theme `<link>` is hot-reloaded with a cache-busting query param.
