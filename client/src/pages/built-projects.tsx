            ...
            {paginated.map((p, i) => {
              ...
              return (
                <div key={p.id} data-testid={`card-project-${p.id}`}
                  ...
                  style={{ background: cardBg, border: `1px solid ${p.highlight ? 'transparent' : HAIRLINE}`, padding: isMobile ? '24px 20px' : '36px 32px', display: 'flex', flexDirection: 'column', color: cardTextColor }}>
                  ...
                  {p.url && (
                    <a href={p.url} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', marginTop: 20, fontSize: 11, fontWeight: 600, letterSpacing: '0.16em', textTransform: 'uppercase', color: BRASS_LIGHT }}>View project →</a>
                  )}
                </div>
              );
            })}
